...
public class TiltCollector {
	...
	public void addNewAccelValues(double ax, double ay, double az) {
		...
		if (debug)
			System.out.println("acceleration sum: " + sumAccel);
	...
	/**
	 * @return The smoothed resulting tilt. If there are not enough valid
	 *         readings in a row, all values will be zero
	 */
	public Tilt getSummarizedTilt() {
		...
		res.x = Math.atan(sumX / Math.sqrt(sumZ * sumZ + sumY * sumY))
				* scaleTilt;
		res.z = Math.atan(sumZ / Math.sqrt(sumX * sumX + sumY * sumY))
				* scaleTilt;
	...
	public boolean isAccelInMotion() {
		if (validAccelsRows.size() < 2)
			return true; // euclid sum of accels not =g
		double treshold = 0.5; // each accel will be between 0 and ~10
		if (Math.abs(validAccelsRows.get(0).y - validAccelsRows.get(1).y) > treshold)
			return true;
		if (Math.abs(validAccelsRows.get(0).z - validAccelsRows.get(1).z) > treshold)
			return true;
		if (Math.abs(validAccelsRows.get(0).x - validAccelsRows.get(1).x) > treshold)
			return true;
		return false;
	}
}
