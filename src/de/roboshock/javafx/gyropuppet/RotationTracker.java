...
public class RotationTracker {
	...
	public void addRotate(Point3D axis, double angle) {
		...
		Point3D localAxisX = new Point3D(rot.getMxx(), rot.getMyx(),
				rot.getMzx());
		Point3D localAxisY = new Point3D(rot.getMxy(), rot.getMyy(),
				rot.getMzy());
		Point3D localAxisZ = new Point3D(rot.getMxz(), rot.getMyz(),
				rot.getMzz());
	...
	public void addRotate(double angleX, double angleY, double angleZ) {
		...
		if (showTorsoRotations) {
			...
			// System.out.printf("   ---   %3.0f    %3.0f ", sensorTiltX,
			// sensorTiltY);
			System.out.print("\n");
		}
	}
	...
	public void applyCorrection(double sensorTiltX, double sensorTiltZ) {
		...
		if (debug)
			System.out.printf("%3.2f  (%3.2f)      %3.2f  (%3.2f)    ",
					virtAngelX, accelAngelX, virtAngelZ, accelAngelZ);

		double xCorr = accelAngelX + virtAngelX; // all values are in degree (0
													// .. 45 .. 180)
		double zCorr = accelAngelZ + virtAngelZ; // all values are in degree (0
													// .. 45 .. 180)
