...
public class DeviceReaderThread extends Thread {
        ...
        try {
        	...
        	while (true) {
        		try {
					...
					System.out.println("Movement during sensor initialize phase ... retrying.");
					// o.k., try again
				}
        	}
			...
			while (! terminationRequested) {
				...
				double scaleGyro = 9.0/7.0 / 1000d; // lead to degree 0 .. 90 .. 180
				double scaleAccel = 6.0 / 10000d;  // -->  acceleration values in g (9.81)
				...
				if (debug) {
					System.out.print(String.format("%-6d  %-6d  %-6d    gx(Bias): %-6d (%-6d)(%-6d)   gy(Bias): %-6d (%-6d)(%-6d)   gz(Bias): %-6d (%-6d)(%-6d) \n", 
          			(int)rotate.x, (int)rotate.y, (int)rotate.z,
          			(int)gx, (int)gyroBias.x, (int)(gx-gyroBias.x), 
          			(int)gy, (int)gyroBias.y, (int)(gy-gyroBias.y), 
          			(int)gz, (int)gyroBias.z, (int)(gz-gyroBias.z)
          			));
				}

			    /**
			     * ax, ay, az: acceleration in m/s² (e.g. summarized 9.81)
			     * gx, gy, gz: Changes in degree (0 .. 45 .. 180) since last reading, relative to the sensor axes
			     *             (actually orientation change speed in degrees per 20 ms, to be precise)
			     */
				if (valueSink6 != null) {
					valueSink6.setNewValues(ax * scaleAccel, ay * scaleAccel, az * scaleAccel,
	...
	private void finetuneBias() throws MovementException {
		...
		for (int i=0; i < 100 ; i++) {
			nextReading();
			if (Math.abs(gx - gyroBias.x) > gyroNoise ) throw new MovementException();
			if (Math.abs(gy - gyroBias.y) > gyroNoise ) throw new MovementException();
			if (Math.abs(gz - gyroBias.z) > gyroNoise ) throw new MovementException();
			sumx += gx;
			sumy += gy;
			sumz += gz;
		}
