...
public class GyroPuppet extends Application implements ValueSinkDouble6 {
    ...
    public Parent createUI() {
    	...
    	exit.setOnAction( (ActionEvent arg0) -> { try { System.exit(0); } 
    ...
    private Slider createSlider(final double value, final String labelText, Rotate r, Pane ui) {
    ...
    public void start(Stage stage) throws Exception {
        ...
        SubScene subScene = new SubScene(createRobot(), width, height, true, SceneAntialiasing.BALANCED);
    ...
    public void stop() throws Exception {
    	...
    	if (readerThread != null) readerThread.terminate();
    	super.stop();
    }
     ...
     */
    public void setNewValues(double ax, double ay, double az, double gx, double gy, double gz) {
    	Platform.runLater(new Runnable() {
			public void run() {
				tiltCollector.addNewAccelValues(ax, ay, az);
			    /* tilt.x, tilt.z : absolute tilt values in degree (0 .. 45 .. 180)
			     * tilt.valid == false  -> invalid tilt due to sensor movement
			     *     (when the euclid sum of ax,ay,az was not around g (9.81) in the last n readings)  */
				TiltCollector.Tilt tilt = tiltCollector.getSummarizedTilt();

				rotationTracker.addRotate(gy-gyroDrift.y, -gx+gyroDrift.x, gz-gyroDrift.z);
				
		        double angleLimit = 60.0;
				if (tilt.valid && Math.abs(tilt.x) < angleLimit && Math.abs(tilt.z) < angleLimit) {
